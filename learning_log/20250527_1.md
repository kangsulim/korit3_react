...
TS 상의 함수를 고려했을 때, 매개변수 item의 자료형은 Item, return 타입은 void.
구현부가 없다는 점에서 추상 메서드 같은 느낌이 난다.


12. 새로운 쇼핑 항목이 item에 저장되고 사용자가 입력한 값이 포함될 수 있도록 코드 작성하기.

addItem 함수는 props에서 가지고 와서 props.addItem(item)을 통해 호출하면 된다.

또한 모달 대화 상자를 추가하면 닫는다.


13. items 상태에 저장도 되고, handleClose도 되는데 items 내용이 표시가 안 됨
...
addItem에 대한 추가적인 List 부분을 작성하여 화면 상에 노출되도록 하였다.
`<List></List> <ListItem></ListItem> <ListItemText></ListItemText>`는 각각 `<table></table> <tr></tr> <td></td>`의 역할을 한다.

내용이 더 길어졌는데 왜 굳이 바꿔 쓰는가?

> css를 건드리지 않아도 되서.
> 예를 들어 'ListItem'의 속성으로 divider를 작성해주면 보조선이 생긴다.


그리고 Button도 속성이 있는데,
AddItem.tsx의 Button 속성

Button 컴포넌트의 속성
- text - default 값으로 그냥 텍스트만 보임
- outlined - 외곽선 표시
- contained - 내부를 default 색으로 채워줌

shoppingList done
- 요약
  - MUI 컴포넌트는 HTML 태그를 대신할 수 있다
    - 이름을 다시 외워야 하지만 css를 포함하고 있어 not bad
  
  - 프로젝트 전체 구조를 파악하는 것이 중요하다
    - shoppingList를 개조해서 to-do-list 등 제작 가능