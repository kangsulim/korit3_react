...
상태는 useState 훅 함수를 이용하여 만들 수 있다. 이 함수는 상태의 초기값인 argument를 하나 받고, 두 element로 구성된 배열을 반환한다. 첫 번째 argument는 상태의 이름이고, 두 번째 argument는 상태의 값을 업데이트하는 데 이용하는 함수다.
형식
```jsx
// const 선언하고 초기화
